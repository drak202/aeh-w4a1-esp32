...
// === SETPOINT z RX frame (byte 19) ===
// potvrdené z RAW dumpov (°C)
uint8_t setpoint = uart_buf[19];
...
int raw_cur = ((Device_Status*)uart_buf)->indoor_temperature_status;

// ošetrenie neplatných hodnôt
if (raw_cur > 0 && raw_cur < 50 && raw_cur != 0x80 && raw_cur != 0xFF) {
    current_temperature = raw_cur;
}
...
void update() override
{
    ...
    int hz = ((Device_Status*)uart_buf)->compressor_frequency;

    set_sensor(compressor_frequency, hz);
    ...
    float power_w = 0.0f;
    if (hz > 0) {
        power_w = hz * 30.0f;   // odhad
    }
    set_sensor(power, power_w);
    ...
    int hum = ((Device_Status*)uart_buf)->indoor_humidity_status;
    if (hum >= 0 && hum <= 100) {
        set_sensor(indoor_humidity_status, hum);
    }
}
...
private:
    float heat_tgt_temp = 23.0f;
    float cool_tgt_temp = 23.0f;
...
int get_response(const uint8_t input, uint8_t *out)
{
    ...
    // ochrana proti overflow
    if (buf_idx >= UART_BUF_SIZE) {
        reset = true;
    }
    // start frame
    else if (buf_idx == 1 && input != 0xF4) {
        reset = true;
    }
    else if (buf_idx == 2 && input != 0xF5) {
        reset = true;
    }
    // length byte
    else if (buf_idx == 5) {
    ...
    // end of frame
    else if (msg_size > 0 && buf_idx == msg_size) {

        // footer check
        if (buf[msg_size - 2] != 0xF4 || buf[msg_size - 1] != 0xFB) {
            reset = true;
        } else {

            memcpy(out, buf, msg_size);

            ESP_LOGD("aircon_climate", "Received %d bytes.", msg_size);
